...
function updateTextFile(templateFile, destinationFile, templateData, delimObj) {
  ...
  if (this.fs.exists(destinationFile)) {
    ...
    var startIndex = templateFileContents.indexOf(delimObj.startTag);
    var endIndex = templateFileContents.indexOf(delimObj.endTag);
...
function copyToolTemplates(arrayOfTemplateFiles, templateData) {
  var tData = templateData || _.merge({}, this.getGlobalConfig(), {resources: this.getResources()});
  ...
  (arrayOfTemplateFiles || []).forEach(fileDef => {
    ...
    fileOperationFn.call(
      this,
      this.toolTemplatePath(fileDef.src),
