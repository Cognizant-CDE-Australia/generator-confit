...
function buildConfig(genToolConfig, successCb) {
  ...
  this.setNpmDevDependenciesFromArray(genToolConfig.packages);
  this.addNpmTasks(genToolConfig.tasks);
  ...
  this.registerBuildToolCommands('install', genToolConfig.onInstall);
...
function writeGeneratorConfig(genResourceData) {
  buildConfig.call(this, genResourceData, () => {
    let templateData = _.merge(
      ...
      this.getGlobalConfig(),
      { resources: this.getResources() },
      { name: this.appPackageName, configFile: this.configFile }
    );
