...
module.exports = {
  ...
  writeGeneratorConfig: writeGeneratorConfig
};
...
function writeGeneratorConfig(genResourceData) {
  // Define the template data we will need
  ...
  this.setNpmDevDependenciesFromArray(genResourceData.packages);
  this.addNpmTasks(genResourceData.tasks);
  this.ts.addTypeLibsFromArray(genResourceData.typeLibs);
  this.addNpmConfig(genResourceData.npmConfig);
  ...
  this.registerBuildToolCommands('install', genResourceData.onInstall);
  this.registerBuildToolCommands('end', genResourceData.onEnd);
