...
let buildToolNameCache = new Set();   // A cache of the buildTool names e.g. NPM, webpack
...
function buildConfig(configModule, templateData, copyTemplateFn) {
  ...
  let testPackages = (configModule.testPackages || []).filter((pkg) => !pkg.isDependency && (pkg.global === 'both' || !pkg.global));
 ...
 * @return {Boolean}                       Indicates whether the config was processed or not
 * @this generator
 */
function writeBuildToolConfig(buildToolResourceData, optionalTemplateData) {
  return writeCommonConfig.call(this, buildToolResourceData, optionalTemplateData, this.copyToolTemplates);
}
...
function writeGeneratorConfig(genResourceData, optionalTemplateData) {
  return writeCommonConfig.call(this, genResourceData, optionalTemplateData, this.copyGeneratorTemplates);
}


/**
 * Processes the commonResourceData and any configModules or toolModules that are direct descendents of the commonResourceData
 * @param {Object} commonResourceData     Resource data (templates, packages)
...
function writeCommonConfig(commonResourceData, optionalTemplateData, copyTemplateFn) {
  ...
  if (commonResourceData.condition === undefined || this.evalExpression(commonResourceData.condition) === 'true') {
    childConfigs = commonResourceData.configModules || commonResourceData.toolModules || [];
  }
