...
function updateTextFile(templateFile, destinationFile, templateData, delimObj) {
  ...
  if (!delimObj) {
    delimObj = FILE_TYPE_MAP[path.extname(templateFile)] ||
                FILE_TYPE_MAP[path.extname(templateFile.replace(path.extname(templateFile), ''))] || // For files called 'yaml.tpl' -> 'yaml'
                JS_BLOCK_DELIMS;
  }
...
function copyTemplateFiles(arrayOfTemplateFiles, templateData, srcPathFn) {
  ...
  (arrayOfTemplateFiles || []).forEach((fileDef) => {
    // If there is a condition which when rendered using EJS evaluates to false, skip this file
    if (this.evalExpression(fileDef.condition) === false) {
      return;
    }
 ...
 * @return {string}          The filtered path, or an environment error
 * @this generator
 */
 ...
 * @return {Boolean|String}     Returns true if the path is valid, or a string indicating the error
 */
