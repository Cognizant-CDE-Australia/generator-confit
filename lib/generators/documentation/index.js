...
let docGenDefaults = {};
const GITHUB = 'GitHub';      // TODO: move this into the resource file as it is also used in publish.js.tpl
...
module.exports = confitGen.create({
  initializing: {
    init: function() {
      ...
      Object.assign(docGenDefaults, resources.defaults, this.getConfig());
    }
  },
  ...
  prompting: function() {
    ...
    this.log(chalk.underline.bold.green('Documentation Generator'));

    let resources = this.getResources().documentation;

    // Filter out GitHub as a publishing option if the project is not a GitHub project
    let publishMethods = resources.publishMethods.filter((options) => this.isProjectHostedOnGitHub() || options.value !== GITHUB);

    // If the project is hosted on GitHub, set GitHub as the default publishing method
    docGenDefaults.publishMethod = this.isProjectHostedOnGitHub() ? GITHUB : docGenDefaults.publishMethod;
