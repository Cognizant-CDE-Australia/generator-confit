...
let vrtDefaults = {};   // Used in multiple task-blocks, so that's why it's here
...
module.exports = confitGen.create({
  initializing: {
    init: function() {
      ...
      vrtDefaults = _.merge({}, resources.defaults, this.getConfig());
    }
  },
  ...
  prompting: function() {
    ...
    this.log(chalk.underline.bold.green('Visual Regression Testing Generator'));
    ...
    let prompts = [
      ...
      },
      {
        ...
        default: vrtDefaults.referenceImageDir,
        validate: this.validatePath,
        when: answers => answers.enabled
      }
    ];
  ...
  configuring: function() {
    ...
    if (config.enabled) {
      config.moduleTestDir = this.filterPath(config.moduleTestDir, 'moduleTestDir', {moduleTestDir: vrtDefaults.moduleTestDir});
      config.referenceImageDir = this.filterPath(config.referenceImageDir, 'referenceImageDir', {referenceImageDir: vrtDefaults.referenceImageDir});
    }
