...
/**
 * Adds a key-value to the config block in package.json
 *
 * @param arrayOfKeyValues {array}
 */
function addNpmConfig(arrayOfKeyValues) {
  ...
  values.forEach((item) => {
    let newItem = this.forEJSInObj(item, config);   // Parse any EJS templates in the object
    let key = _.keys(newItem)[0];
    _.set(packageJson, 'config.' + key, newItem[key]);
  });
